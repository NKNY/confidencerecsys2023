...
# Minor modifications by Knyazev marked with `MODIFIED`.
...
class NDCGMetric(_RankingMetric):
  ...
  def _compute_impl(self, labels, predictions, weights, mask):
    """See `_RankingMetric`."""
    predictions = tf.cast(predictions, tf.float32)  # MODIFIED: Cast to float32 as float64 model would fail
    weights = tf.cast(weights, tf.float32)  # MODIFIED: Cast to float32 as float64 model would fail
    topn = tf.shape(predictions)[1] if self._topn is None else self._topn
    sorted_labels, sorted_weights = utils.sort_by_scores(
        predictions, [labels, weights], topn=topn, mask=mask)
